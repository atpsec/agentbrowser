...
function renderTemplates(){
  ...
  list.forEach(t=>{const card=document.createElement('article');card.className='template-card';const h=document.createElement('strong');h.textContent=t.title;const meta=document.createElement('div');meta.className='template-meta';meta.textContent=`${families.find(f=>f.id===t.family)?.label} · ${t.level} · ~${t.minutes} dk`;const b=document.createElement('button');b.className='button';b.textContent='Bu şablonla başla';b.addEventListener('click',()=>{state.family=t.family;$('#projectTitle').value=t.title;$('#purpose').value=`${t.title} için küçük, anlaşılır ve çalışan bir uygulama oluştur.`;$$('input[name=family]').forEach(x=>x.checked=x.value===t.family);save({family:t.family,title:t.title});$('#projectTitle').scrollIntoView({behavior:'smooth',block:'center'})});card.append(h,meta,b);$('#templateGrid').append(card)});
}
